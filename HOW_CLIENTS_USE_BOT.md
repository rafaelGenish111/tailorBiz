...
## 📋 1.5 טופס צור קשר → WhatsApp + בוט (אוטומציה)

### איך זה עובד?

כשמישהו **משאיר פרטים בטופס צור קשר** (או בדף נחיתה), מתבצעת אוטומציה:

1. הליד נשמר במערכת (לקוח/ליד חדש או עדכון ליד קיים).
2. נשלחת אוטומטית **הודעת WhatsApp** ללקוח עם הודעת ברוכים הבאים.
3. **שיחה עם הבוט מתחילה** – כשהלקוח עונה ב-WhatsApp, הבוט מגיב וממשיך איתו את השיחה.

### תהליך:

```
👤 לקוח ממלא טופס צור קשר (שם, טלפון, אימייל, הודעה)
    ↓
🖥 המערכת שומרת את הליד ומפעילה אוטומציה
    ↓
📱 נשלחת הודעת WhatsApp: "שלום [שם], תודה שפנית אלינו! קיבלנו את הפרטים..."
    ↓
👤 הלקוח עונה ב-WhatsApp
    ↓
🤖 הבוט עונה וממשיך את השיחה (קביעת פגישה, שאלות, העברה לנציג וכו')
```

### מה צריך לעשות?

**כלום!** האוטומציה מופעלת אוטומטית עבור:

- טופס צור קשר באתר (`leadSource: website_form`)
- טופס בדף נחיתה (`leadSource: landing_page_campaign`)

חובה ש-**WhatsApp מחובר** וש-**מספר הטלפון** של הלקוח תקין (מאומת ב-WhatsApp).
...
- [x] **WhatsApp** - כבר עובד!
- [x] **טופס צור קשר** - אוטומציה שולחת WhatsApp ומפעילה בוט אוטומטית
- [ ] **Chat Widget** - הוסף `<ChatWidget />` לאפליקציה
- [ ] **בדיקה** - נסה לשלוח הודעה מהאתר / למלא טופס צור קשר
